...
```{r}
kiva_us <- kiva[kiva$country == "United States",]
```
...
```{r}
# your code here
kiva_manufacture <- kiva[kiva$sector == "Manufacturing",]
kiva_manufacture
```
...
```{r}
plot(kiva$funded_amount, kiva$loan_amount)
```
...
```{r}
kiva_phill <- kiva[kiva$country =="Philippines",]
head(kiva_phill)
```

```{r}
library(lubridate)
kiva_phill$funded_time <- ymd_hms(kiva_phill$funded_time)
kiva_phill$posted_time <- ymd_hms(kiva_phill$posted_time)
tail(kiva_phill)
```
...
```{r}
min(kiva_phill$posted_time)
max(kiva_phill$posted_time)
```
...
```{r}
kiva_phill$funding_duration <- difftime(kiva_phill$funded_time, kiva_phill$posted_time, units = "hours")
sample(kiva_phill$funding_duration, size = 5)
```
...
```{r}
kiva_phill$posted_time_month <- month(kiva_phill$posted_time, label = T)
sample(kiva_phill$posted_time_month, size = 5)
```
...
```{r}
loan_agg <- aggregate(funding_duration ~ repayment_interval + posted_time_month, data = kiva_phill, FUN = mean)
loan_agg[loan_agg$funding_duration == max(loan_agg$funding_duration),]
```
...
```{r}
library(ggplot2)
 ggplot(data = loan_agg, aes(x = posted_time_month, y = funding_duration, color = repayment_interval, group = repayment_interval))+
   geom_line()+
   geom_point()+
